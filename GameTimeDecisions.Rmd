...
pbp_fourth %>% group_by(season, posteam) %>% summarise(attempts = n(), went_total = sum(went_for_it), rate = went_total/attempts) %>% arrange(desc(rate)) %>% filter(year != 2025)
...
wf_train = wf_with_features %>% filter(season %in% train_seasons)
wf_test = wf_with_features %>% filter(season %in% test_seasons)
...
auc_val <- as.numeric(pROC::auc(wf_test$fourth_down_converted, wf_test$pred_prob))
logloss <- -mean(wf_test$fourth_down_converted * log(pmax(wf_test$pred_prob, 1e-15)) +
                 (1 - wf_test$fourth_down_converted) * log(pmax(1 - wf_test$pred_prob, 1e-15)))
brier   <- mean((wf_test$pred_prob - wf_test$fourth_down_converted)^2)
acc_05  <- mean(wf_test$pred_class_05 == wf_test$fourth_down_converted)

list(AUC = auc_val, LogLoss = logloss, Brier = brier, Acc_at_0_5 = acc_05)
